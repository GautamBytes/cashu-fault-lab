...
const walletDoctorSchemas = await Promise.all(
  [
    'nip60-capture.schema.json',
    'nip60-diagnosis.schema.json',
    'nip60-repair-plan.schema.json',
    'nip60-check.schema.json',
  ].map((name) => text(`spec/schemas/${name}`)),
);
const walletDoctorSpecDigest = digest(walletDoctorSchemas.join('\0'));
...
await writeExpected(
  'packages/wallet-doctor-contract/src/generated/metadata.ts',
  `/* Generated by Cashu Fault Lab deterministic wallet-doctor schema projection.
 * specDigest: ${walletDoctorSpecDigest}
 * Do not edit by hand.
 */
...
await writeExpected(
  'packages/wallet-doctor-contract/generated/rust/Cargo.toml',
  `[package]
name = "cashu_fault_lab_wallet_doctor_contract"
version = "0.2.0"
edition = "2021"
description = "Generated Cashu Fault Lab wallet-doctor artifact contract"
...
await writeExpected(
  'packages/wallet-doctor-contract/generated/rust/src/lib.rs',
  `//! Generated Cashu Fault Lab wallet-doctor artifact contract.
//! specDigest: ${walletDoctorSpecDigest}

pub const SPEC_DIGEST: &str = "${walletDoctorSpecDigest}";
...
await writeExpected(
  'packages/wallet-doctor-contract/generated/python/cashu_fault_lab_wallet_doctor_contract/__init__.py',
  `"""Generated Cashu Fault Lab wallet-doctor artifact contract."""

from dataclasses import dataclass
from typing import Optional

__spec_digest__ = "${walletDoctorSpecDigest}"
...
await writeExpected(
  'packages/wallet-doctor-contract/generated/python/cashu_fault_lab_wallet_doctor_contract/py.typed',
  '',
);

await writeExpected(
  'packages/wallet-doctor-contract/generated/README.md',
  `# Generated Wallet Doctor Contract Artifacts

These deterministic TypeScript, Rust, and Python files expose version and digest interoperability
metadata for the schemas in \`spec/schemas/nip60-*.schema.json\`. They are not complete generated
models; the JSON Schemas remain the normative wire contracts.

- Capture schema version: \`2\`
- Combined schema digest: \`${walletDoctorSpecDigest}\`
