...
test('home is accessible, has one visible title, and fits the viewport', async ({
  ...
  await expect(demoCommand.getByText('npx cashu-fault-lab@0.1.4 demo')).toBeVisible();
  await expect(copyCommand).toBeVisible();
  await expect(demoCommand.getByText('npx cashu-fault-lab@0.1.4 demo')).toHaveCSS(
    'white-space',
    'nowrap',
  );
