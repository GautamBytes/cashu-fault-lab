...
describe('home components', () => {
  it('keeps GitHub in the hero and promotes scenario discovery', async () => {
    ...
    expect(within(hero).getByText('npx --yes cashu-fault-lab@0.2.0 demo')).toBeVisible();
  ...
  it('copies the single-line demo command without release metadata', async () => {
    ...
    expect(within(command).getByText('npx --yes cashu-fault-lab@0.2.0 demo')).toHaveAttribute(
      'tabindex',
      '0',
    );
    expect(
    ...
    expect(await navigator.clipboard.readText()).toBe('npx --yes cashu-fault-lab@0.2.0 demo');
