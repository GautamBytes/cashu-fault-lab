  ...
  it.each(supportedAdapterLanguages)(
    ...
    async (language) => {
      ...
      try {
        ...
        expect(text).toContain('cashu-fault-lab@0.1.3 adapter preflight');
        expect(text).toContain('cashu-fault-lab@0.1.3 adapter preview');
        const workflow = await readFile(join(output, '.github/workflows/ci.yml'), 'utf8');
        expect(workflow).toContain('permissions:\n  contents: read');
        expect(workflow).toContain('cashu-fault-lab@0.1.3 adapter preflight');
        expect(workflow).toContain(`--adapters adapter-manifest.json`);
        expect(workflow).toContain(
          `${language === 'typescript' ? 'pnpm start' : language === 'rust' ? 'cargo run' : 'uvicorn'}`,
        );
