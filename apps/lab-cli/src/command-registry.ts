...
export function createCommandRegistry(): readonly CliCommandDefinition[] {
  return [
    ...
    },
    {
      ...
      options: [
        { flags: '--json', description: 'Emit machine-readable JSON instead of text.' },
        {
          flags: '--suite <suite>',
          description: 'Limit checks to a lab suite.',
          defaultValue: 'all',
    ...
    {
      name: 'lifecycle run',
      usage: 'cashu-fault-lab lifecycle run <scenario>',
      summary: 'Run one wallet lifecycle fault scenario',
      arguments: [{ value: '<scenario>', description: 'Lifecycle scenario ID or JSON path.' }],
      options: [
        { flags: '--adapter <id>', description: 'Lifecycle adapter ID.' },
        { flags: '--mint <id>', description: 'Test mint identity.' },
        { flags: '--mint-url <url>', description: 'Override the scenario mint URL.' },
        {
          flags: '--seed <seed>',
          description: 'Deterministic seed supplied out of band for replay.',
          defaultValue: 'cashu-fault-lab-lifecycle',
        },
        {
          flags: '--format <format>',
          description: 'Redacted lifecycle report format.',
          defaultValue: 'json',
          choices: ['json', 'junit', 'html'],
        },
        { flags: '--output <path>', description: 'Write the lifecycle report.' },
        { flags: '--artifact <path>', description: 'Legacy JSON output alias.' },
      ],
      examples: [
        'cashu-fault-lab lifecycle run mint-response-lost --adapter cashu-ts --mint nutshell-local --seed demo',
      ],
      env: [
        'CFL_LIFECYCLE_<ADAPTER>_URL',
        'CFL_LIFECYCLE_<ADAPTER>_TOKEN',
        'CFL_HTTP_FAULT_GATEWAY_URL',
        'CFL_HTTP_FAULT_GATEWAY_TOKEN',
      ],
      modes: ['json', 'junit', 'html'],
    ...
    },
    {
      ...
      usage: 'cashu-fault-lab lifecycle matrix',
      summary: 'Run the wallet lifecycle compatibility matrix',
      arguments: [],
      options: [
        {
          flags: '--profile <profile>',
          description: 'Lifecycle matrix profile.',
          defaultValue: 'wallet-lifecycle-v1',
        },
        {
          flags: '--seed <seed>',
          description: 'Deterministic seed.',
        ...
        { flags: '--json', description: 'Emit machine-readable matrix results.' },
        { flags: '--output <path>', description: 'Write matrix output.' },
      ],
      examples: ['cashu-fault-lab lifecycle matrix --profile wallet-lifecycle-v1 --json'],
      env: ['CFL_LIFECYCLE_MATRIX_FILE'],
      modes: ['text', 'json'],
      artifacts: [],
      exitCodes: COMMON_EXIT_CODES,
    },
    {
      ...
      usage: 'cashu-fault-lab lifecycle replay <artifact>',
      summary: 'Replay an exact wallet lifecycle failure',
      arguments: [{ value: '<artifact>', description: 'Redacted failure artifact JSON.' }],
      options: [
        { flags: '--seed <seed>', description: 'Original seed supplied out of band.' },
        { flags: '--adapter <id>', description: 'Lifecycle adapter ID.' },
        { flags: '--mint <id>', description: 'Test mint identity.' },
      ],
      examples: [
      ...
      env: ['CFL_LIFECYCLE_<ADAPTER>_URL', 'CFL_LIFECYCLE_<ADAPTER>_TOKEN'],
      modes: ['text'],
      artifacts: [],
      exitCodes: COMMON_EXIT_CODES,
    },
    ...
    {
      name: 'wallet-doctor diagnose',
      usage: 'cashu-fault-lab wallet-doctor diagnose <capture>',
      summary: 'Explain why relays disagree about one wallet',
      arguments: [{ value: '<capture>', description: 'Capture bundle JSON.' }],
      options: [
        { flags: '--output <path>', description: 'Write the diagnosis artifact.' },
        { flags: '--format <format>', description: 'Diagnosis report format (json).' },
      ],
      examples: ['cashu-fault-lab wallet-doctor diagnose artifacts/wallet-doctor/capture.json'],
      env: [],
      modes: ['text', 'json'],
    ...
    {
      name: 'wallet-doctor plan',
      usage: 'cashu-fault-lab wallet-doctor plan <capture>',
      summary: 'Emit a dry-run repair plan with safety invariants',
      arguments: [{ value: '<capture>', description: 'Capture bundle JSON.' }],
      options: [{ flags: '--output <path>', description: 'Write the repair plan artifact.' }],
      examples: ['cashu-fault-lab wallet-doctor plan artifacts/wallet-doctor/capture.json'],
      env: [],
      modes: ['text'],
    ...
    {
      name: 'wallet-doctor run',
      usage: 'cashu-fault-lab wallet-doctor run <scenario>',
      summary: 'Run one wallet-doctor scenario against a live fixture/relay/mint stack',
      arguments: [
        { value: '<scenario>', description: 'Packaged scenario id or relative JSON path.' },
      ],
      options: [
        { flags: '--seed <seed>', description: 'Deterministic seed.' },
        { flags: '--output <path>', description: 'Write the scenario artifact.' },
      ],
      examples: ['cashu-fault-lab wallet-doctor run del-chain-break --seed demo'],
      env: [
        'CFL_WALLET_DOCTOR_FIXTURE_URL',
        'CFL_WALLET_DOCTOR_FIXTURE_TOKEN',
        'CFL_WALLET_DOCTOR_RELAYS',
        'CFL_WALLET_DOCTOR_RELAY_CONTROLS',
        'CFL_WALLET_DOCTOR_RELAY_CONTROL_TOKEN',
      ],
      modes: ['text'],
    ...
    {
      name: 'wallet-doctor matrix',
      usage: 'cashu-fault-lab wallet-doctor matrix',
      summary: 'Run every packaged wallet-doctor scenario',
      arguments: [],
      options: [
        { flags: '--profile <profile>', description: 'Matrix profile (nip60-doctor-v1).' },
        { flags: '--seed <seed>', description: 'Deterministic seed.' },
        { flags: '--json', description: 'Emit machine-readable matrix results.' },
        { flags: '--output <path>', description: 'Write matrix output.' },
      ],
      examples: ['cashu-fault-lab wallet-doctor matrix --profile nip60-doctor-v1 --json'],
      env: [
        'CFL_WALLET_DOCTOR_FIXTURE_URL',
        'CFL_WALLET_DOCTOR_FIXTURE_TOKEN',
        'CFL_WALLET_DOCTOR_RELAYS',
    ...
    {
      name: 'wallet-doctor replay',
      usage: 'cashu-fault-lab wallet-doctor replay <artifact>',
      summary: 'Replay a wallet-doctor scenario artifact with its original seed',
      arguments: [{ value: '<artifact>', description: 'Scenario artifact JSON.' }],
      options: [{ flags: '--seed <seed>', description: 'Original seed supplied out of band.' }],
      examples: [
