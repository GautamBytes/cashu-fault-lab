...
\`\`\`bash
${context.tokenEnv}=local-token npx --yes cashu-fault-lab@0.1.3 adapter preflight --adapters adapter-manifest.json
${context.tokenEnv}=local-token npx --yes cashu-fault-lab@0.1.3 adapter preview --adapters adapter-manifest.json --sender ${context.name} --receiver ${context.name} --output-dir cashu-fault-results
\`\`\`
    ...
    steps:
      ...
      - name: Adapter contract preflight
        run: |
          ...
          ${context.tokenEnv}=adapter-ci-local-token npx --yes cashu-fault-lab@0.1.3 adapter preflight --adapters adapter-manifest.json
      - run: docker build .
`,
  };
}
