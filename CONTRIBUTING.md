...
1. Generate a standalone scaffold:
   `npx --yes cashu-fault-lab@0.2.0 adapter init --language typescript --name your-wallet`.
...
4. Run the read-only contract check:
   `npx --yes cashu-fault-lab@0.2.0 adapter preflight --adapters ./your-wallet/adapter-manifest.json`.
5. Run the response-loss and duplicate-delivery preview:
   `npx --yes cashu-fault-lab@0.2.0 adapter preview --adapters ./your-wallet/adapter-manifest.json --sender your-wallet --receiver your-wallet`.
